...
public class DoubleCombinedMethodAction extends ActionSupport {

	private  HttpServletRequest request = ServletActionContext.getRequest();
	private 	HttpServletResponse response = ServletActionContext.getResponse();
	private DoubleCombinedDao doublecombinedDaoimpl = new com.textUser.dao.impl.DoubleCombinedDaoimpl();
	...
	//	A:红，1\nB:123,1
		...
		int Dindex;//索引
		Dindex=random.nextInt(dataList.size());//在0与集合元素个数之间随机生成一个数,随机生成一个索引
		//选择两两组合的index，相邻的两个参数
		int min;
		int max;
	...
	//	count.put("zhong",Math.abs(index[0]-index[1])-1);
		List<String[]> caselist =new ArrayList<String[]>();//创建名为caselist的数组
		String [] usecase= new String [dataList.size()];

		for(int i=0;i<dataList.get(min).size();i++)
		{
			usecase[min]=dataList.get(min).get(i);//dataList.get(min)取出datalist的第min个元素
	...
	public void saveOrUpdateCombined() throws Exception{
		...
		if(list.get(0).getFlag().indexOf("doublecombined")==-1)
		{
			for(int i=0;i< list.size();i++)
			{
				String flag =list.get(i).getFlag();
				list.get(i).setFlag("doublecombined"+flag);
			}
		}
		List<Long> ids=doublecombinedDaoimpl.saveOrUpdateCombinedCase(list);
		response.setContentType("application/json;charset=UTF-8");
	    response.setHeader("pragma", "no-cache");
	...
	public String outputCombinedExcel() throws IOException{
		...
		doublecombinedDaoimpl.outputExcel(os, filename, titles, "doublecombined"+record);
		if (os != null) {
			os.close();
		}
		return null;				
	...
	public void deleteRow()
	{
		...
		doublecombinedDaoimpl.deleteRow(id);
	}
	/**
	 * 
	 * 
	...
	public void searchCase(){
		...
		List<CaseBean> result =new ArrayList<CaseBean>();
		result =doublecombinedDaoimpl .showCase(flag);
		List<Map<String, Object>> resultList =new ArrayList<Map<String, Object>>();
	...
	public void showCombobox()
	{
		...
		List<CaseBean> result =new ArrayList<CaseBean>();
		result=doublecombinedDaoimpl.showCombobox();
		...
		{
			if(caseBean.getFlag().indexOf("doublecombined")!=-1)
			{
		...
		{
			Map<String, Object> m =new HashMap<String, Object>();
			String flag = result3.get(i).getFlag().replace("doublecombined", "");
			m.put("flag", flag);
			m.put("date", flag);			
			list.add(m);
		}
