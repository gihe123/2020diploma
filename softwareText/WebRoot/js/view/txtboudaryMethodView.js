...
var centerGirdPanel = Ext.create('Ext.grid.Panel', {
	...
	tbar:[ {  
		...
		//		window.location.href=contextPath + '/BoundaryMethodAction!outputEquivalentExcel.action'
				Ext.Ajax.request({
					url : contextPath + '/BoundaryMethodAction!saveOrUpdateBoundary.action',
					method : 'POST',
					timeout : 30000,
			...
			click : function() {
	    		...
	    		listeners : {
	    			click : function() {
	    				...
	    				var count =store.getCount();
	    				var flag ="txtboundary,"+store.getAt(0).data.flag;
	    	    ...
	    	    valueField:"date",//这个是隐藏的变量
	      	    ...
	      	    listeners:{
	    	  	   	...
	    	  	   	//	Ext.example.msg(record[0].data.date,combo.value+"--"+combo.getValue(),true,50000);
	    	  	   		store.proxy.url=contextPath + '/BoundaryMethodAction!searchCase.action?flag='+combo.getValue(); 	   
	    	  	   	 minLength = 0;
	    	  		 maxLength = 0;
	    	  		 isMark =  0;
	    	  		 isChar =  0;
...
function loadData(){	
	...
	formStore.proxy.url =contextPath + '/BoundaryMethodAction!txtBoundaryMethod.action';
...
function canel() {
    Ext.MessageBox.confirm('提示', '确定删除该记录?', function(btn){
        ...
        if(row[0].data.id!=null){
    	    Ext.Ajax.request({
    			url : contextPath + '/BoundaryMethodAction!deleteRow.action',
    			method : 'POST',
    			timeout : 30000,
    			type : 'ajax',	    					// form:"ajaxform"
    			params : {
