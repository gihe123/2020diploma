...
import com.textUser.dao.ComboEquivalentDao;
import com.textUser.dao.impl.ComboEquivalentDaoimpl;
import com.util.FileUtil;
import com.util.Jacksonutil;



public class ComboEquivalentMEthodAction extends ActionSupport {


	private static final long serialVersionUID = -2870542636449444008L;

	ComboEquivalentDao comboEquivalentDaoimpl =new ComboEquivalentDaoimpl();
	...
	public String outputEquivalentExcel() throws IOException{
		...
		comboEquivalentDaoimpl.outputExcel(os, filename, titles, "comboequivalent"+record);
		if (os != null) {
			os.close();
		}
		return null;				
	}
	public void comboEquivalentMethod() throws Exception
	{
		Date date =new Date();
		...
		if(msgs.length>=3){
			...
			resultlist.add(new CaseBean(null, msgs[random.nextInt(msgs.length-2)+1], now));			
	...
	public void searchCase(){
		...
		result =comboEquivalentDaoimpl.showCase(flag);
		response.setContentType("application/json;charset=UTF-8");
		response.setHeader("cache-control", "no-cache");
		response.setCharacterEncoding("UTF-8");
	    Map<String, Object> m =new HashMap<String, Object>();
	...
	public void saveOrUpdateEquivalent() throws Exception
	{
		...
		if(list.get(0).getFlag().indexOf("comboequivalent")==-1)
		{
			for(int i=0;i< list.size();i++)
			{
				String flag =list.get(i).getFlag();
				list.get(i).setFlag("comboequivalent"+flag);
			}
		}
		List<Long> ids=comboEquivalentDaoimpl.saveOrUpdateEquivalentCase(list);
		response.setContentType("application/json;charset=UTF-8");
	...
	public void showCombobox()
	{
		...
		List<CaseBean> result =new ArrayList<CaseBean>();
		result=comboEquivalentDaoimpl.showCombobox();
		...
		{
			if(caseBean.getFlag().indexOf("comboequivalent")!=-1)
			{
		...
		{
			Map<String, Object> m =new HashMap<String, Object>();
			String flag = result3.get(i).getFlag().replace("comboequivalent", "");
			m.put("flag", flag);
			m.put("date", flag);			
			list.add(m);
		}
