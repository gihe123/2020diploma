...
import com.textUser.dao.DoubleCombinedDao;
import com.textUser.dao.ThirdCombinedDao;
import com.util.FileUtil;
import com.util.Jacksonutil;
import com.util.UserUtil;

public class ThirdCombinedMethodAction extends ActionSupport {

	private  HttpServletRequest request = ServletActionContext.getRequest();
	private 	HttpServletResponse response = ServletActionContext.getResponse();
	private ThirdCombinedDao thirdcombinedDaoimpl = new com.textUser.dao.impl.ThirdCombinedDaoimpl();
	...
	//	A:阮，1\nB:123,1
		...
		int Dindex;
		Dindex=random.nextInt(dataList.size());
		//选择三三组合的index，相邻的三个参数
		int min;
		int mid;
		int max;
		if(Dindex==(dataList.size()-1))
		{
			max=Dindex;
		}else if(Dindex==(dataList.size()-2))
		{
			max=Dindex+1;
		}else
		{
		...
		//三三组合							
	//	count.put("zhong",Math.abs(index[0]-index[1])-1);
		List<String[]> caselist =new ArrayList<String[]>();
		String [] usecase= new String [dataList.size()];

		for(int i=0;i<dataList.get(min).size();i++)
		{
			usecase[min]=dataList.get(min).get(i);
			for(int z=0;z<dataList.get(mid).size();z++){				
				usecase[mid]=dataList.get(mid).get(z);
				for(int j=0;j<dataList.get(max).size();j++)
				{
					usecase[max]=dataList.get(max).get(j);				
					caselist.add(usecase.clone());
				}
			}
		}
		
		for(int i=0;i<caselist.size();i++){
			for(int j=0;j<caselist.get(i).length;j++)
			{
				if(caselist.get(i)[j]==null)
				{
					caselist.get(i)[j]=dataList.get(j).get(random.nextInt(dataList.get(j).size()));
				}
			}
			//生成以逗号隔开
			String str="" ;
			for(int z=0;z<caselist.get(i).length;z++)
			{
				
				if(z==caselist.get(i).length-1){
					str+=caselist.get(i)[z];
					
				}else{
					str+=caselist.get(i)[z]+",";
				}
			}
			resultList.add(new CaseBean(null, str, now));
		}
		...
		if (selectStr.trim() != "") {
			...
			List<CaseBean>tempList =new ArrayList<CaseBean>();
			tempList.addAll(resultList);
			//通过正则表达式筛选出最终结果
			for(CaseBean caseBean :tempList)
			{				
				for(String reg: falseRegex)
				{
					if(caseBean.getUser().matches(reg)==true)
					{
						resultList.remove(caseBean);
						break;
					}					
				}
			}
		...
		for(CaseBean casebean: resultList)
		{
	...
	public void saveOrUpdateCombined() throws Exception{
		...
		if(list.get(0).getFlag().indexOf("thirdcombined")==-1)
		{
			for(int i=0;i< list.size();i++)
			{
				String flag =list.get(i).getFlag();
				list.get(i).setFlag("thirdcombined"+flag);
			}
		}
		List<Long> ids=thirdcombinedDaoimpl.saveOrUpdateCombinedCase(list);
		response.setContentType("application/json;charset=UTF-8");
	    response.setHeader("pragma", "no-cache");
	...
	public String outputCombinedExcel() throws IOException{
		...
		thirdcombinedDaoimpl.outputExcel(os, filename, titles, "thirdcombined"+record);
		if (os != null) {
			os.close();
		}
		return null;				
	...
	public void deleteRow()
	{
		...
		thirdcombinedDaoimpl.deleteRow(id);
	}
	/**
	 * 
	 * 
	...
	public void searchCase(){
		...
		List<CaseBean> result =new ArrayList<CaseBean>();
		result =thirdcombinedDaoimpl .showCase(flag);
		List<Map<String, Object>> resultList =new ArrayList<Map<String, Object>>();
	...
	public void showCombobox()
	{
		...
		List<CaseBean> result =new ArrayList<CaseBean>();
		result=thirdcombinedDaoimpl.showCombobox();
		...
		{
			if(caseBean.getFlag().indexOf("thirdcombined")!=-1)
			{
		...
		{
			Map<String, Object> m =new HashMap<String, Object>();
			String flag = result3.get(i).getFlag().replace("thirdcombined", "");
			m.put("flag", flag);
			m.put("date", flag);			
			list.add(m);
		}
