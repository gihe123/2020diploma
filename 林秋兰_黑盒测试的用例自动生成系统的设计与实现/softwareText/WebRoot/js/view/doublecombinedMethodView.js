	...
	tbar:[{ //历史查询方法重写
		    ...
		    valueField:"date",//这个是隐藏的变量
	  	    ...
	  	    listeners:{
	  	   		...
	  	   		detailModel=[];
	  	   			Ext.Ajax.request({
					url : contextPath + '/DoubleCombinedMethodAction!searchCaseColumn.action',
					method : 'POST',
					timeout : 30000,
...
//			window.location.href=contextPath + '/BoundaryMethodAction!outputEquivalentExcel.action'
			Ext.Ajax.request({
				url : contextPath + '/DoubleCombinedMethodAction!saveOrUpdateCombined.action',
				method : 'POST',
				timeout : 30000,
				type : 'ajax',	    					// form:"ajaxform"
				params : {
		...
		//			 Ext.Msg.alert("1",store.getAt(0).data.id+"=="+store.getAt(1).data.id);
			...
			listeners : {
				click : function() {
					...
					var flag ="doublecombined,"+store.getAt(0).data.flag;
					var count =store.getCount();
					for(var i=0;i<count;i++){
	...
	listeners : {
		click : function() {
			...
			for(var i=0;i<count;i++){
				...
				// 后台不会再识别到[objcet,object]这样的
				// 而是具体数值			
			window.location.href=contextPath + '/DoubleCombinedMethodAction!outputCombinedExcel.action?record='+record.data.flag;
			...
			Ext.Msg.confirm('系统提示','该操作会强制覆盖当前数据，操作不可逆。是否继续？',function(btn){
				if(btn == 'yes') {
					form.submit({
						...
						waitMsg : '正在上传文件...',// 提示信息
						waitTitle : '信息提示',
						url : contextPath+ '/DoubleCombinedMethodAction!impDataExcel.action',
						params : {},
						method : 'post',
...
function selectChange(msg,selection){
	...
	//获取报表列信息
	Ext.Ajax.request({
		url: contextPath + '/DoubleCombinedMethodAction!getColumnForGirdpanel.action',
	    params: {
	    	msg:msg
...
function showhistoryGrid(msg){
	...
	detailStore = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url: contextPath + '/DoubleCombinedMethodAction!searchCase.action',
			reader: {
				type: 'json',
				root: 'root'
			},
...
function showGrid(msg,selection){
	...
	detailStore = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url: contextPath + '/DoubleCombinedMethodAction!combinedMethod.action',
			reader: {
				type: 'json',
				root: 'root'
			},
...
function canel() {
    Ext.MessageBox.confirm('提示', '确定删除该记录?', function(btn){
        ...
        if(row[0].data.id!=null||row[0].data.id!=""){
    	    Ext.Ajax.request({
    			url : contextPath + '/DoubleCombinedMethodAction!deleteRow.action',
    			method : 'POST',
    			timeout : 30000,
    			type : 'ajax',	    					// form:"ajaxform"
    			params : {
