...
var centerGirdPanel = Ext.create('Ext.grid.Panel', {
	...
	tbar:[{
		...
		//		window.location.href=contextPath + '/BoundaryMethodAction!outputEquivalentExcel.action'
				Ext.Ajax.request({
					url : contextPath + '/ComboBoundaryMethodAction!saveOrUpdateBoundary.action',
					method : 'POST',
					timeout : 30000,
			...
			//			 Ext.Msg.alert("1",store.getAt(0).data.id+"=="+store.getAt(1).data.id);
	    		...
	    		listeners : {
	    			click : function() {
	    				var store = this.up("grid").getStore();		    		
	    				var flag ="comboboundary,"+store.getAt(0).data.flag;
	    				var count =store.getCount();
	    				for(var i=0;i<count;i++){
		...
		listeners : {
			click : function() {
				...
				for(var i=0;i<count;i++){
					...
					// 后台不会再识别到[objcet,object]这样的
					// 而是具体数值			
				window.location.href=contextPath + '/ComboBoundaryMethodAction!outputBoundaryExcel.action?record='+record.data.flag;
	    	    ...
	    	    valueField:"date",//这个是隐藏的变量
	      	    ...
	      	    listeners:{
	    	  	   	...
	    	  	   	//	Ext.example.msg(record[0].data.date,combo.value+"--"+combo.getValue(),true,50000);
	    	  	   		store.proxy.url=contextPath + '/ComboBoundaryMethodAction!searchCase.action?flag='+combo.getValue(); 	   
	    		  		 msg = null;
			...
			Ext.Msg.confirm('系统提示','该操作会强制覆盖当前数据，操作不可逆。是否继续？',function(btn){
				if(btn == 'yes') {
					form.submit({
						...
						waitMsg : '正在上传文件...',// 提示信息
						waitTitle : '信息提示',
						url : contextPath+ '/ComboBoundaryMethodAction!impDataExcel.action',
						params : {},
						method : 'post',
...
function loadData(){	
	...
	formStore.proxy.url =contextPath + '/ComboBoundaryMethodAction!comboBoundaryMethod.action';
...
function canel() {
    Ext.MessageBox.confirm('提示', '确定删除该记录?', function(btn){
        ...
        if(row[0].data.id!=null||row[0].data.id!=""){
    	    Ext.Ajax.request({
    			url : contextPath + '/ComboBoundaryMethodAction!deleteRow.action',
    			method : 'POST',
    			timeout : 30000,
    			type : 'ajax',	    					// form:"ajaxform"
    			params : {
